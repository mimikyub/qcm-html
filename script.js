document.addEventListener('DOMContentLoaded', () => {
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr id="row-${i}-${generatedTables}">
                                <td>${i}</td>
                                <td> <button class="alt-button" onclick="toggleInputOrCheckbox(${i}, ${generatedTables})">T</button></td>
                                ...
                                <td>
                                    <button hidden class="plus-button" onclick="addColumns(${i}, ${generatedTables})">+</button>
                                    <button hidden class="minus-button" onclick="removeColumns(${i}, ${generatedTables})">-</button>
    ...
    window.toggleInputOrCheckbox = function(rowNumber, tableNumber) {
        ...
        if (actionButton.innerText === 'T') {
            row.replaceChild(inputCell, row.cells[2]);
            ...
            actionButton.innerText = 'C';
        } else if (actionButton.innerHTML === 'C') {
            row.replaceChild(document.createElement('td'), row.cells[2]);
            row.cells[2].innerHTML = checkboxes;
            row.querySelector('.plus-button').style.display ='hidden';// 'inline-block';
            row.querySelector('.minus-button').style.display ='hidden';// 'inline-block';
            actionButton.innerText = 'T';
        }
    };
